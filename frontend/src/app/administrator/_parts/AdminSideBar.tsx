...
export function AdminSideBar({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      ...
      className="w-12 hover:w-60 ease-in-out duration-300 h-fit color-white text-white  shrink-0"
    >
      <div className="flex items-center gap-3 shrink-0 absolute top-5 left-2">
        ...
        <div className={`flex flex-col px-5 ${showSideBar ? "" : "hidden"}`}>
          <span className="text-sm font-bold leading-none text-foreground">Super Admin</span>
          <span className="text-[11px] text-foreground mt-0.5">
            Flow AI
          </span>
        </div>
      </div>
