...
export function ContentCalendar({
  ...
  const selectedPosts = selectedDate
    ? posts.filter((p) => p.scheduledDate.slice(0, 10) === selectedDate)
    : [];
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-70 shrink-0">
            <div className="flex items-center justify-between mb-3">
              <button
                onClick={prevMonth}
                className="p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors"
            ...
            <div className="grid grid-cols-7 mb-1">
              {WEEK_DAYS.map((wd) => (
                <div
                  key={wd}
                  className="text-center text-xs font-medium text-slate-400 dark:text-gray-500 py-1"
                >
                  {wd}
                </div>
              ))}
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      isSelected
                        ? "bg-[#5048e5] text-white font-semibold"
                        : isToday
          ...
          <div className="flex-1 border-t md:border-t-0 md:border-l border-slate-100 dark:border-gray-800 pt-4 md:pt-0 md:pl-6 overflow-y-auto max-h-105">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-gray-200 mb-3">
              Постын дэлгэрэнгүй
              {selectedDate && (
                <span className="ml-2 text-slate-400 dark:text-gray-500 font-normal">
                ...
                {displayedPosts.map((post, i) => (
                  <div
                    ...
                    style={{
                      ...
                      transform:
                        i < visiblePosts || visiblePosts === 0
                          ? "translateY(0)"
                          : "translateY(8px)",
                    }}
                  >
                    <PostCard post={post} images={images} onSaved={onSaved} />
                  </div>
                ))}
              </div>
            )}
