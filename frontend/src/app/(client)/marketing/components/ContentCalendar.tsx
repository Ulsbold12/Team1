    ...
    <Card className="rounded-2xl shadow p-0">
      <CardHeader className="px-6 pt-6 pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl font-bold text-slate-900">Контент хуанли</CardTitle>
          {posts.length > 0 && (
            <button
              onClick={onReset}
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-70 shrink-0">
            <div className="flex items-center justify-between mb-3">
              <button onClick={prevMonth} className="p-1.5 rounded-md hover:bg-slate-100 transition-colors" aria-label="Өмнөх сар">
                <ChevronLeft className="size-4 text-slate-600" />
              </button>
              <span className="text-sm font-semibold text-slate-700">
                {year} оны {MONGOLIAN_MONTHS[month]}
              </span>
              <button onClick={nextMonth} className="p-1.5 rounded-md hover:bg-slate-100 transition-colors" aria-label="Дараах сар">
          ...
          <div className="flex-1 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 overflow-y-auto max-h-105">
            <h3 className="text-sm font-semibold text-slate-700 mb-3">
              Постын дэлгэрэнгүй
              {selectedDate && <span className="ml-2 text-slate-400 font-normal">— {selectedDate.slice(0, 10)}</span>}
            </h3>
            ...
            {selectedDate && selectedPosts.length === 0 && (
              <p className="text-sm text-slate-400">Энэ өдөрт пост байхгүй байна.</p>
            )}

            {displayedPosts.length > 0 && (
              <div className="flex flex-col gap-3">
                {!selectedDate && (
                  <p className="text-xs text-slate-500 mb-1">Нийт {posts.length} пост үүслээ.</p>
                )}
