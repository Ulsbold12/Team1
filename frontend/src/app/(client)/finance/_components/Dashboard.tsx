...
export const Dashboard = ({ aiResult }: { aiResult?: any }) => {
  ...
  const displayExpense = aiResult?.expenses
    ? aiResult.expenses.reduce((s: number, c: AiCategory) => s + (c.total ?? 0), 0)
    : expense;
  ...
  const cards = [
    {
      ...
      percentage: displayRevenue >= displayExpense ? `+${((displayRevenue / (displayExpense || 1)) * 100 - 100).toFixed(1)}%` : "0%",
    ...
    },
    {
      ...
      percentage: displayExpense > 0 ? `-${((displayExpense / (displayRevenue || 1)) * 100).toFixed(1)}%` : "0%",
    ...
    },
    {
      ...
      progress: displayRevenue > 0 ? Math.min(Math.round((displayNetProfit / displayRevenue) * 100), 100) : 0,
    },
  ];
  ...
  const pieData =
    aiResult?.expenses && aiResult.expenses.length > 0
      ? aiResult.expenses.map((c: AiCategory, i: number) => ({ id: i, name: c.name, value: c.total }))
      : records.length > 0
      ? [
