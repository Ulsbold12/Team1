...
const QUARTERS = [
  {
    label: "I улирал (1–3-р сар)",
    value: 1,
    months: [1, 2, 3],
    deadline: "4-р сарын 20",
  },
  {
    label: "II улирал (4–6-р сар)",
    value: 2,
    months: [4, 5, 6],
    deadline: "7-р сарын 20",
  },
  {
    label: "III улирал (7–9-р сар)",
    value: 3,
    months: [7, 8, 9],
    deadline: "10-р сарын 20",
  },
  {
    label: "IV улирал (10–12-р сар)",
    value: 4,
    months: [10, 11, 12],
    deadline: "1-р сарын 20",
  },
];
...
function isDeadlineSoon(
  ...
  const now = new Date();
  const deadlineMonth = quarter.value === 4 ? 0 : quarter.value * 3;
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width="130" height="130" viewBox="0 0 130 130">
        ...
        <text x="65" y="61" textAnchor="middle" fontSize="11" fill="#6b7280">
          Нийт
        </text>
        <text
          x="65"
...
export default function TaxPage() {
  ...
  const filtered = records.filter((r) => {
    ...
    return (
      ...
      quarter.months.includes(d.getMonth() + 1)
    );
  });
...
<body>
  ...
  <table>
    ...
    <tr><td><b>НӨАТ</b></td><td>Нэмэгдсэн өртгийн албан татвар</td><td>${fmt(revenue)}</td><td>10%</td><td><b>${fmt(vat)}</b></td></tr>
    <tr><td><b>ААН</b></td><td>Аж ахуйн нэгжийн татвар</td><td>${fmt(netProfit > 0 ? netProfit : 0)}</td><td>10%</td><td><b>${fmt(cit)}</b></td></tr>
    <tr><td><b>НД</b></td><td>Нийгмийн даатгал (${employeeCount} ажилчин)</td><td>${fmt(totalSalary)}</td><td>21%</td><td><b>${fmt(socialInsurance)}</b></td></tr>
    <tr class="total-row"><td colspan="4">Нийт татвар</td><td>${fmt(totalTax)}</td></tr>
  </table>
  <div class="footer">
    ...
    // ← PDF татах notification
    addNotification({
      category: "finance",
      title: "Татварын тайлан татагдлаа",
      desc: `${selectedYear} он ${quarter.label} — ${fmt(totalTax)}`,
      icon: Download,
  ...
  async function handleSend() {
    ...
    // ← Тайлан үүсгэх notification
    addNotification({
      category: "finance",
      title: "Татварын тайлан үүслээ",
      desc: `${selectedYear} он ${quarter.label} · Нийт ${fmt(totalTax)}`,
      icon: FileText,
      iconColor: "text-[#5048e5]",
      iconBg: "bg-[#5048e5]/10",
    });
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div
          className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border ${deadlineBadge}`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <TaxCard
          label="НӨАТ"
          sublabel="Нэмэгдсэн өртгийн албан татвар"
          rate="10%"
          base={fmt(revenue)}
          amount={vat}
          color="blue"
        ...
        <TaxCard
          label="ААН"
          sublabel="Аж ахуйн нэгжийн татвар"
          rate="10%"
          base={fmt(netProfit > 0 ? netProfit : 0)}
          amount={cit}
          color="violet"
        />
        <TaxCard
          label="НД"
          sublabel="Нийгмийн даатгал"
          rate="21%"
          base={fmt(totalSalary)}
              ...
              {filtered.map((r) => (
                ...
                >
                  <td className="px-6 py-3 text-muted-foreground">
                    {r.month
                      ? new Date(r.month).toLocaleDateString("mn-MN", {
                          year: "numeric",
                          month: "long",
                        })
                      : "—"}
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-xs font-semibold px-2 py-1 rounded-full border ${badge}`}
        >
          {rate}
        </span>
      </div>
