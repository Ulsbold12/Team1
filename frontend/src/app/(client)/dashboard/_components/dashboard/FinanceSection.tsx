...
export function FinanceSection() {
  ...
  const revenueProgress = revenue > 0 ? Math.min((revenue / (revenue + expense)) * 100, 100) : 0;

  const maxExpense = Math.max(...records.map((r) => r.expense ?? 0), 1);
  const expenseBars = records.slice(0, 5).map((r) =>
    Math.round(((r.expense ?? 0) / maxExpense) * 100)
  );
  while (expenseBars.length < 5) expenseBars.push(0);

  const maxRevenue = Math.max(...records.map((r) => r.revenue ?? 0), 1);
  const revenueBars = records.slice(0, 6).map((r) =>
    Math.round(((r.revenue ?? 0) / maxRevenue) * 100)
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
          ...
          <div className="flex items-baseline gap-2 mt-2">
            ...
            <span className="text-xs font-bold text-emerald-500">+{profitPct}%</span>
          </div>
          <div className="mt-4 h-1.5 w-full bg-muted rounded-full overflow-hidden">
            <div className="bg-[#5048e5] h-full rounded-full" style={{ width: `${revenueProgress}%` }} />
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-2xl font-bold text-foreground">{fmt(expense)}</span>
            <span className="text-xs font-bold text-rose-500">-{expensePct}%</span>
          </div>
        ...
        <div className="lg:col-span-2 bg-card p-6 rounded-xl border border-border shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className={`text-2xl font-bold ${netProfit >= 0 ? "text-foreground" : "text-rose-500"}`}>
